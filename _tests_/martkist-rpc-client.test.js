...
import MartkistParityRegistry from '../src/martkist-rpc-client-parity-registry';
...
describe('Core Parity', () => {
    ...
    beforeAll(async() => {
        ...
        parityRegistry = new MartkistParityRegistry(client);
    });
...
describe('Network Services', () => {
    ...
    it('Martkistd is running on regtest', async() => {
    ...
    it('Martkistd is running on version 3.0.6.2', async() => {
    ...
    it('Can reserve alias - first half of the process to create an alias', async() => {
        let stepOne = await client.walletServices.alias.new(
            { aliasName: newAliasId, 
                ...
                encryptionPublicKey: "", 
                witness: ""});
        let stepTwo = await client.walletServices.martkistTxFund(stepOne);
        let stepThree = await client.transactionServices.signRawTransaction(stepTwo);
        let stepFour = await client.walletServices.martkistSendRawTransaction(stepThree.hex);
...
describe('Wallet Services', () => {
    ...
    it('martkistListReceivedByAddress returns more than 1 result', async() => {
       let listLength = await client.walletServices.martkistListReceivedByAddress();
