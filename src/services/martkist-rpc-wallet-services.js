...
import { walletAliasServices } from './wallet/martkist-rpc-wallet-alias-services'
import { walletAssetAllocationServices } from './wallet/martkist-rpc-wallet-asset-allocation-services';
import { walletAssetServices } from './wallet/martkist-rpc-wallet-asset-services';
import { walletCertificateServices } from './wallet/martkist-rpc-wallet-certificate-services';
import { walletEscrowServices } from './wallet/martkist-rpc-wallet-escrow-services';
import { walletOfferServices } from './wallet/martkist-rpc-wallet-offer-services';

export function martkistRpcWalletServices(callRpc, utilityServices) {
    return {
    ...
    async function importPrivKey(martkistPrivKey, label, rescan) {
        ow(martkistPrivKey, ow.string.label("importPrivKey:martkistPrivKey").not.empty);
        if(label) {
    ...
    async function pruneMartkistServices() {
        return await callRpc('prunemartkistservices');
    }
    ...
    async function martkistDecodeRawTransaction(hexString) {
        ow(hexString, ow.string.label("martkistDecodeRawTransaction:hexString").not.empty);
        return await callRpc('martkistdecoderawtransaction', [hexString]);
    }

    async function martkistListReceivedByAddress() {
        return await callRpc('martkistlistreceivedbyaddress');
    }

    async function martkistSendRawTransaction(hexString, allowHighFees, instantSend) {
        ow(hexString, ow.string.label("martkistSendRawTransaction:hexString").not.empty);
        if(allowHighFees) {
            ow(allowHighFees, ow.boolean.label("martkistSendRawTransaction:allowHighFees").is(x => x == true || x == false));
        }
        if(instantSend) {
            ow(instantSend, ow.boolean.label("martkistSendRawTransaction:instantSend").is(x => x == true || x == false));
        }
        return await callRpc('martkistsendrawtransaction', arguments);
    }

    async function martkistTxFund(hexString, addresses) {
        ow(hexString, ow.string.label("martkistTxFund:hexString").not.empty);
        if(addresses){
            ow(addresses, ow.object.label("martkistTxFund:address").not.empty);
        }
        return await callRpc('martkisttxfund', arguments);
    }
