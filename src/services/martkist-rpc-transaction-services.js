...
export function martkistRpcTransactionServices(callRpc) {
    return {
    ...
    async function createRawTransaction({inputs, outputs, lockTime=0} = {}) {
        ...
        ow(lockTime, ow.number.label("createRawTransaction:lockTime").greaterThanOrEqualTo(0));
    ...
    async function signRawTransaction({hexString,
        ...
        if (previousTransactionOutputs && privateKeys) {
            ...
            return await callRpc('signrawtransaction',
                [hexString,previousTransactionOutputs,privateKeys,signatureHashType]);
        }
